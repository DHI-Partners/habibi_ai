...
def check_quote(quote, context, now):
	...
	if quote.get("sales_order"):
		return "done"
	message_at = context.get("message_at")
	answered_at = quote.get("answered_at")
	if (
		quote.get("turn_id") == context.get("turn_id")
		or not answered_at
		or not message_at
		or message_at <= answered_at
	):
		# Главное правило: клиент ответил после того, как расчёт до него
		# дошёл. Проверяет код, а не инструкция, которую модель может забыть.
		# answered_at ставится только после фактической отправки ответа с
		# расчётом. «Другой ход» недостаточно: «да», написанное вдогонку, пока
		# бот ещё считал или отправлял, открывает новый ход, хотя суммы клиент
		# не видел; так же выглядит повтор хода после сбоя движка.
		raise Refusal(
