...
def check_quote(quote, context, now):
	...
	if quote.get("turn_id") == context.get("turn_id") or not message_at or message_at <= quote["creation"]:
		# Главное правило: после расчёта должно прийти сообщение клиента.
		# Проверяет код, а не инструкция, которую модель может забыть. «Другой
		# ход» — недостаточно: «да», пришедшее, пока бот ещё считал, открывает
		# новый ход, хотя расчёта клиент не видел; так же выглядит повтор хода
		# после сбоя движка. Поэтому сравнивается время сообщения клиента.
		raise Refusal(
